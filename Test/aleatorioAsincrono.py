...
num_iteraciones = 100  # Número de veces que se seleccionarán elementos al azar
...
async def main():
    async with aiohttp.ClientSession() as session:
        ...
        for _ in range(num_iteraciones):
            name, url_value = random.choice(list(urls.items()))
            for policy in policies:
                ...
                tasks.append(post_request(session, url + pathCircuit, data))
